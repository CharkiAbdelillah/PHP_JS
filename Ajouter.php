...
//$target = "images/".basename($image);
		if (move_uploaded_file($image,$target)) {
  		$msg = "Image telecharger avec success";
  	}else{
  		$msg = "Impossible de telecharger l'image";
  	}
  	$sql = "INSERT INTO automobile (IMM, MARQUE,PRIX_LOC,PHOTO) VALUES ('$imm','$marque','$prixloc', '$target')";
		$resultat=mysqli_query($cnloccar,$sql);

if($resultat)
{
	echo "Insertion des donnees validees";
}else{
